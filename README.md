...
`--squash` 把一些不必要的commit进行压缩，在master建立一个新的提交点。例如：当前分支开发时commit很乱，在合并时不希望把这些历史commit带过来，于是使用`--squash`进行合并，此时文件已经同合并后一样了，但是不移动HEAD、不提交，而是相当于在master建立一个新的提交点。需要进行一次额外的commit来"总结"一下，然后完成最终的合并。
...
将指定的版本号的commit提交合并到当前分支（当主分支版本大于当前分支版本时可以这样写）
...
注意：push操作会同时操作此远程仓库名对应的所有url

##### 18.1、【git push [-u] 远程仓库名 本地分支名[:远程分支名]】
...
注意：使用 `git branch --unset-upstream`取消关联，以后就不能直接使用`git push`；使用`git branch --set-upstream-to 本地分支名 远程仓库名/远程分支名`

##### 18.2、【git push -d 远程仓库名 分支名】
...
标签相当于某个commit版本的别名，使用`git tag`查看所有标签（按照标签字典排序展示）

##### 19.1、【git tag 标签名 [-m "说明"] [版本名]】

给指定版本设置标签，不写版本名时默认是当前版本

##### 19.2、【git show 标签名】

查看指定标签名对应的commit版本的详细信息

##### 19.3、【git tag -d 标签名】

删除指定标签

##### 19.4、【git push 远程仓库名 标签名】

将本地创建的标签推送到远程仓库

##### 19.5、【git push 远程仓库名 :refs/tags/标签名】

先删除本地标签，再删除远程仓库的指定标签
